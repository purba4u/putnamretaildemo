...
try {
	...
	'Launching login form and enter email test data'
    ...
    TestObject Logout = findTestObject('DCIO/Logout')

    if (WebUI.waitForElementPresent(Logout, 10)) {
        KeywordUtil.markPassed('DCIO Login is Successful')
    } else {
        KeywordUtil.markFailed('DCIO Login is failed.')
    }
}
catch (Exception ex) {
