...
try {
    WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.AdvisorPath) + 'mutual-funds/closed-end-funds/')
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        String statusCode = resp.getStatusCode()
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Advisor Closed end funds', rowIndex, FullFundNameSite, FundNameDropdown, ManagerRows, FundDescription, Objective, StrategyAndProcess, FundFacts, 'N/A')
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Advisor Closed end funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
