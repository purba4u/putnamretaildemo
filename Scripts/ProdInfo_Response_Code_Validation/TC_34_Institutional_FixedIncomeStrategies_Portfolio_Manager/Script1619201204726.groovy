...
try {
    //CustomKeywords.'customKeywords.PortalUIKeywords.loadUrl'((GlobalVariable.ENV + GlobalVariable.InstitutionalPath) + 'investment-capabilities/strategies/core-bond')
    WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.InstitutionalPath) + 'investment-capabilities/strategies/core-bond')
    ...
    for (int index = 0; index <= (NoOfFunds - 1); index++) {
        ...
        if (statusCode.equals('200')) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Inst Fixed Income Strategies', index, 'N/A', 'N/A', ManagerInfo, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
			PrevUrl = url
        } else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Inst Fixed Income Strategies', index, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
