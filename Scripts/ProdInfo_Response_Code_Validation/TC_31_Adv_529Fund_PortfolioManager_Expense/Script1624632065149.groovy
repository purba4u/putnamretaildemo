    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
		...
		if ((PrevUrl != url)&&(index != 1)) {
			...
			if (statusCode.equals('200')) {
				...
				CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Advisor 529 funds-PE', index, ManagerInfo, ExpenseRatio, SalesCharges, cdscSales, trailComm, 'N/A', 'N/A', 'N/A')
				PrevUrl = url
			} else {
				CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Advisor 529 funds-PE', index, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
