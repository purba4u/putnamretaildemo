...
try {
	...
	WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.DCIOPath) + 'mutual-funds/funds/95-stable-value-fund')
	...
	if (!AllShareClass.equals(null)) {
        ...
        for (int sindex = 0; sindex <= (NoOfShareClass - 1); sindex++) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'DCIO Open end funds', rowIndex, FullFundNameSite, 'N/A', ManagerRows, FundDescription, Objective, StrategyAndProcess, FundFacts, 'N/A', PerformanceAnnual, PerformanceCumulative, AnnualPerf, PerfSnapshot)
			PrevUrl = url
			} else {
