...
try {
	WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.IndividualPath) + 'annuities/portfolios/185-fixed-account')
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        if (statusCode.equals('200')) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Ind Annuity All Adv funds', index, FundNameDropdown, FullFundNameSite, ManagerInfo, FundDescription, Objective, AnnutySalesStory, FundFacts, Expense, PerformanceAnnual, AnnuityCumPerf, AnnuityAnnPerf, AnnuityRestPerf)
			PrevUrl = url
        } else {
