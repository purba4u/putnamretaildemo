...
try {
    //CustomKeywords.'customKeywords.PortalUIKeywords.loadUrl'((GlobalVariable.ENV + GlobalVariable.IndividualPath) + 'mutual-funds/')
	...
	String ProdUrl
	String PrevUrl
	String FundDescriptionUAT
	String FundDescriptionProd
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        for (int sindex = 0; sindex <= (NoOfShareClass - 1); sindex++) {
            ...
            FundDescriptionUAT = WebUI.getText('IndividualInvestor/PageSpecificObjects/ProdInfo/FundDescription')
			ProdUrl = url.replace("www-uat.putnam.com", "www.putnam.com")
			WebUI.openBrowser(ProdUrl)
			FundDescriptionProd = WebUI.getText('IndividualInvestor/PageSpecificObjects/ProdInfo/FundDescription')
			if (FundDescriptionProd == FundDescriptionUAT) {
				KeywordUtil.markPassed(url + 'Fund Description' + FundDescriptionUAT)
			}
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Ind Open end funds', rowIndex, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Ind Open end funds', rowIndex, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
