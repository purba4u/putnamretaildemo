...
try {
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        for (int sindex = 0; sindex <= (NoOfShareClass - 1); sindex++) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Ind Open end funds', rowIndex, FullFundNameSite, FundNameComp, ManagerRows, FundDescription, Objective, StrategyAndProcess, FundFacts, ExpenseRatio)
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Ind Open end funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Share class dropdown missing', 'Failed', 'Ind Open end funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
			WebUI.navigateToUrl(PrevUrl)
			rowIndex = rowIndex + 1
		}
    }
