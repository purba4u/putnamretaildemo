...
try {
    //CustomKeywords.'customKeywords.PortalUIKeywords.loadUrl'((GlobalVariable.ENV + GlobalVariable.InstitutionalPath) + 'investment-capabilities/strategies/emerging-markets-equity/')
    WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.InstitutionalPath) + 'investment-capabilities/strategies/emerging-markets-equity/')
    ...
    for (int index = 0; index <= (NoOfFunds - 1); index++) {
        ...
        if (statusCode.equals('200')) {
            ManagerInfo = WebUI.getText(findTestObject('IndividualInvestor/PageSpecificObjects/ProdInfo/ManagerNameStrategy'), FailureHandling.CONTINUE_ON_FAILURE)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Inst Equity Strategies', index, 'N/A', 'N/A', ManagerInfo, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
			PrevUrl = url
        } else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Inst Equity Strategies', index, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A')
