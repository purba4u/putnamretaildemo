...
try {
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        for (int sindex = 0; sindex <= (NoOfShareClass - 1); sindex++) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'DCIO Open end funds', rowIndex)
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'DCIO Open end funds', rowIndex)
