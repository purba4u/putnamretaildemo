...
try {
	WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.IndividualPath) + 'annuities/portfolios/220-fixed-account-a-plus')
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        if (statusCode.equals('200')) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Ind Annuity All Adv plus funds', index, FundNameDropdown, FullFundNameSite, ManagerInfo, FundDescription, Objective, AnnutySalesStory, FundFacts, Expense, PerformanceAnnual, AnnuityCumPerf, AnnuityAnnPerf, AnnuityRestPerf)
			PrevUrl = url
        } else {
