    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        String statusCode = resp.getStatusCode()
        	if (statusCode.equals('200')) {
				...
				CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Advisor Open end funds-PE', index, ManagerInfo, ExpenseRatio, SalesCharges, cdscSales, trailComm, 'N/A', 'N/A', 'N/A')
				PrevUrl = url
			} else {
            	CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Advisor Open end funds-PE', index, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
