...
try {
    //CustomKeywords.'customKeywords.PortalUIKeywords.loadUrl'((GlobalVariable.ENV + GlobalVariable.IndividualPath) + 'mutual-funds/closed-end-funds/')
	WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.IndividualPath) + 'mutual-funds/closed-end-funds/')
    WebUI.waitForPageLoad(20)

    'Click the first fund on CEF landing page'
    WebUI.click(findTestObject('IndividualInvestor/PageSpecificObjects/CloseEndFundsPage/ClosedEndLandingPage_FirstFund'))
    WebUI.waitForPageLoad(20)
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        String statusCode = resp.getStatusCode()
			...
			FullFundNameSite = WebUI.getText(findTestObject('IndividualInvestor/PageSpecificObjects/ProdInfo/FundName2'), FailureHandling.CONTINUE_ON_FAILURE)
			KeywordUtil.logInfo(FullFundNameSite)
			...
			PerformanceCEF = WebUI.getText(findTestObject('IndividualInvestor/PageSpecificObjects/ProdInfo/PerfCEF'), FailureHandling.CONTINUE_ON_FAILURE)
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Ind Closed end funds', rowIndex, FullFundNameSite, FundNameDropdown, ManagerRows, FundDescription, Objective, StrategyAndProcess, FundFacts, 'N/A', PerformanceCEF, 'N/A', AnnualPerf, PerfSnapshot)
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Ind Closed end funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
