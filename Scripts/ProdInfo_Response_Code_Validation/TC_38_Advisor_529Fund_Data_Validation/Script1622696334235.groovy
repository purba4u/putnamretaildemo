...
try {
    //CustomKeywords.'customKeywords.PortalUIKeywords.loadUrl'((GlobalVariable.ENV + GlobalVariable.AdvisorPath) + '529/portfolios/722-putnam-529-growth/A')
    WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.AdvisorPath) + '529/portfolios/722-putnam-529-growth/A')
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        for (int sindex = 0; sindex <= (NoOfShareClass - 1); sindex++) {
			...
			WebUI.check(findTestObject('IndividualInvestor/PageSpecificObjects/ProdInfo/cumulativeButton'),
				FailureHandling.CONTINUE_ON_FAILURE)
			collegeCumulativePerf = WebUI.getText(findTestObject('IndividualInvestor/PageSpecificObjects/ProdInfo/collegeCumulativePerf'), FailureHandling.CONTINUE_ON_FAILURE)
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Advisor 529 funds', rowIndex, FullFundNameSite, FundNameDropdown, ManagerRows, FundDescription, Objective, 'N/A', FundFacts, ExpenseRatio, PerformanceAnnual, collegeCumulativePerf, AnnualPerf, PerfSnapshot)
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Advisor 529 funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Share class dropdown missing', 'Failed', 'Advisor 529 funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
			WebUI.navigateToUrl(PrevUrl)
			rowIndex = rowIndex + 1
		}
    }
