...
try {
    //CustomKeywords.'customKeywords.PortalUIKeywords.loadUrl'((GlobalVariable.ENV + GlobalVariable.AdvisorPath) + 'mutual-funds/')
	WebUI.navigateToUrl((GlobalVariable.ENV + GlobalVariable.AdvisorPath) + 'mutual-funds/')
    ...
    for (int index = 1; index <= (NoOfFunds - 1); index++) {
        ...
        for (int sindex = 0; sindex <= (NoOfShareClass - 1); sindex++) {
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Passed', 'Advisor Open end funds', rowIndex, FullFundNameSite, FundName, ManagerRows, FundDescription, Objective, StrategyAndProcess, FundFacts, ExpenseRatio, PerformanceAnnual, PerformanceCumulative, AnnualPerf, PerfSnapshot)
			PrevUrl = url
			} else {
            KeywordUtil.markFailed('Server response is ' + statusCode)
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Server response is ' + statusCode, 'Failed', 'Advisor Open end funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
			...
			CustomKeywords.'excelKeywords.WriteExcel.demoKey'(url,  'Share class dropdown missing', 'Failed', 'Advisor Open end funds', rowIndex, 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down', 'page down')
			WebUI.navigateToUrl(PrevUrl)
			rowIndex = rowIndex + 1
		}
    }
